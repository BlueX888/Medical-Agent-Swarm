...
async def deep_research(query: str, max_iterations: int = 2) -> Dict[str, Any]:
    """
    Conduct evidence-oriented medical research.

    Args:
        query: Research question.
        max_iterations: Search breadth hint. It is capped by
            DEEP_RESEARCH_MAX_ITERATIONS to avoid runaway benchmark latency.

    Returns:
        A dict with answer/findings/confidence/sources/evidence_level/status.
    """
...
def format_research_report(query: str, report: Any) -> str:
    ...
    if report.sources:
        output.append("")
        output.append(f"参考来源数量：{len(report.sources)}")

    output.append("")
    output.append("💡 数据来源：网络搜索 + 证据综合")
    return "\n".join(output)
