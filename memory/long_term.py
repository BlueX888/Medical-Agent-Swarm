...
class LongTermMemory:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        try:
            ...
            if not mem0_api_key:
                raise ValueError("MEM0_API_KEY not found. Set it in config.py in the project root")
            ...
            logger.info("To enable Mem0: set MEM0_API_KEY in config.py in the project root")
            self.enabled = False
    ...
    def search_similar_sessions(
        ...
        try:
            ...
            # 添加调试日志
            logger.debug(f"Mem0 search called with limit={limit}")
            ...
            if isinstance(results, dict):
                ...
                logger.debug(f"Mem0 returned dict with {len(results_list)} results (requested limit={limit})")
            elif isinstance(results, list):
                results_list = results
                logger.debug(f"Mem0 returned list with {len(results_list)} results (requested limit={limit})")
            else:
                results_list = []
