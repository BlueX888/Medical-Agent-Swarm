...
try:
    from memory.evolved_skills import (
        SkillStore, SkillMatcher, SkillSynthesizer, SkillEvolver, UsageLogger
    )
    EVOLVED_SKILLS_AVAILABLE = True
except ImportError:
    logger.warning("Evolved Skills module not available")
    EVOLVED_SKILLS_AVAILABLE = False
...
class SwarmCoordinator:
    ...
    def __init__(
        ...
        # Evolved Skills: 自进化 Skill 系统初始化
        self._evolved_skills_enabled = False
        if EVOLVED_SKILLS_AVAILABLE:
            try:
                self._skill_store = SkillStore()
                self._usage_logger = UsageLogger()

                # 复用 MedicalKnowledgeBase 的 embedding model
                ...
                try:
                    ...
                    logger.warning("Could not load embedding model from MedicalKnowledgeBase, skill matching disabled")

                self._skill_matcher = SkillMatcher(
                    skill_store=self._skill_store,
                    embedding_model=embedding_model,
                )
                self._skill_synthesizer = SkillSynthesizer(
                    skill_store=self._skill_store,
                ...
                self._evolved_skills_enabled = True
                logger.info("Evolved Skills system initialized")
            except Exception as e:
                logger.warning(f"Evolved Skills system disabled: {e}")
    ...
    async def process(
        ...
        # Evolved Skills: 匹配相关的 evolved skills 并注入上下文
        injected_skill_ids = []
        if self._evolved_skills_enabled:
            try:
                matched = self._skill_matcher.match(question)
                if matched:
                    injected_skill_ids = [m["skill_id"] for m in matched]
                    skill_prompt = self._skill_matcher.format_for_prompt(matched)
                    enhanced_context["_evolved_skill_prompt"] = skill_prompt
                    enhanced_context["_injected_skill_ids"] = injected_skill_ids
                    self._usage_logger.log_match(
                        session_id=session_id,
                        query=question,
                        matched_skills=[
                            {"skill_id": m["skill_id"], "skill_name": m["skill_name"],
                             "score": m["score"], "injected": True}
                            for m in matched
                        ],
                    )
                    logger.info(f"Matched {len(matched)} evolved skills for prompt injection")
            except Exception as e:
                logger.error(f"Skill matching failed (non-blocking): {e}")
        ...
        # Evolved Skills: 记录会话结果
        if self._evolved_skills_enabled and injected_skill_ids:
            try:
                self._usage_logger.log_session_outcome(
                    session_id=session_id,
                    injected_skill_ids=injected_skill_ids,
                    outcome="positive",
                    session_time_seconds=(end_time - start_time).total_seconds(),
                    metadata={"mode": mode},
                )
            except Exception as e:
                logger.error(f"Skill usage logging failed: {e}")
    ...
    async def _process_with_swarm(
        ...
        # Evolved Skills: 合成新 skill + 记录结果 + 定期检查进化
        if self._evolved_skills_enabled:
            # 记录会话结果
            if injected_skill_ids:
                try:
                    outcome = "negative" if timeout_occurred else "positive"
                    self._usage_logger.log_session_outcome(
                        session_id=session_id,
                        injected_skill_ids=injected_skill_ids,
                        outcome=outcome,
                        session_time_seconds=(end_time - start_time).total_seconds(),
                        metadata={"mode": "swarm", "timeout": timeout_occurred},
                    )
                except Exception as e:
                    logger.error(f"Skill usage logging failed: {e}")
            ...
            if not timeout_occurred:
                try:
                    skill_id = await self._skill_synthesizer.synthesize_from_session(
                        session_id=session_id,
                        question=question,
                        shared_context=shared_context,
                    ...
                    if skill_id:
                        logger.info(f"Synthesized evolved skill: {skill_id}")
                except Exception as e:
                    logger.error(f"Skill synthesis failed (non-blocking): {e}")

            # 10% 概率触发进化检查
            import random
            if random.random() < 0.1:
                try:
                    evolved = await self._skill_evolver.check_and_evolve()
                    if evolved:
                        logger.info(f"Evolved {len(evolved)} skills: {evolved}")
                except Exception as e:
                    logger.error(f"Skill evolution check failed: {e}")
